...
export class TypeExtractor {
  ...
  private _getExposedMembers(c: ts.Symbol): ts.Symbol[] {
  ...
  private _getPublicProperties(c: ts.Symbol): ts.Symbol[] {
    const members: ts.Symbol[] = [];
  ...
  getClassMembers(c: ts.Symbol): ts.Symbol[] {
    ...
    return isInjectable ? this._getExposedMembers(c) : this._getPublicProperties(c);
  }
}
